...
function escapeAsStringLiteral (str) {
  // JSON.stringify always returns double-quote strings; we only need to escape single quotes.
  return JSON.stringify(str).slice(1, -1).replace(/'/, '\\\'')
}
...
export default async function (moduleOptions) {
  ...
  options.escapedThemeColors = escapeAsStringLiteral(JSON.stringify(options.themeColors))
  ...
  options.escapedScript = escapeAsStringLiteral(script)
